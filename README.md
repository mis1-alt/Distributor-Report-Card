...
2. Upload these 5 files to the repository:
   ...
   - `packages.txt`
   - `README.md`
   - `accurex_logo.png` — the app auto-loads this on every report, so no one needs to upload a logo manually anymore.
...
1. Open the app URL
2. Upload your Excel sales file
3. Select distributors to generate
4. Click **Generate PDFs**
5. Download individually or as a ZIP

The Accurex logo is applied automatically to every report from `accurex_logo.png` in the repo — no need to upload it each time. The sidebar upload field is only for overriding the logo on a one-off run.

...
def generate_pdf(dist_name, fy26, fy27, months_elapsed, today_str, month_str, logo_bytes=None):
    buf = io.BytesIO()
    ...
    # Orders table
    orders_table = []
    for p, (q6, a6) in sorted(fy26.items(), key=lambda x: -x[1][1]):
        cy_q, cy_a = fy27[p] if p in fy27 else (0, 0)
        ly_mon_a  = round(a6 / 12)
        cy_mon_a  = round(cy_a / months_elapsed) if months_elapsed else 0
        deg_abs_a = ly_mon_a - cy_mon_a
        order_a   = ly_mon_a - cy_mon_a + deg_abs_a
        orders_table.append((p, q6, a6, ly_mon_a, cy_q, cy_a, cy_mon_a, deg_abs_a, order_a))
    ...
    if logo_bytes:
        ...
        logo = RLImage(logo_buf, width=38*mm, height=14.5*mm)
    ...
    # ── Table 1: Top 5 de-growth ──
    story.append(Paragraph(
        'Product-wise comparison \u2014 Top 5 highest de-growth products (value-wise)',
        PS('sh1', fontName='DVSans-Bold', fontSize=8.5, textColor=DGRAY, spaceAfter=5)))

    if top5:
        t1_hdr = [ph('Product'), ph('FY25\u201326 Qty', TA_RIGHT), ph('FY25\u201326 Value', TA_RIGHT),
                  ph('FY26\u201327 Qty', TA_RIGHT), ph('FY26\u201327 Value', TA_RIGHT),
                  ph('Value Diff', TA_RIGHT), ph('De-growth %', TA_RIGHT), ph('Status', TA_CENTER)]
        t1_data = [t1_hdr]
        for p, q6, a6, q7, a7, vdiff, pct in top5:
            t1_data.append([
                Paragraph(p, PS(f'p1_{p[:6]}', fontSize=7.5, leading=10)),
                pc(str(q6), align=TA_RIGHT),
                pc(inr(a6), align=TA_RIGHT),
                pc(str(q7), align=TA_RIGHT),
                pc(inr(a7), align=TA_RIGHT),
                pc(inr(vdiff), '#A32D2D', TA_RIGHT),
                pc(f'{pct:.1f}%', '#A32D2D', TA_RIGHT),
                pc('De-growth', '#A32D2D', TA_CENTER),
            ])
        t1 = Table(t1_data, colWidths=[62*mm,14*mm,22*mm,14*mm,22*mm,22*mm,18*mm,18*mm], repeatRows=1)
        ...
        story.append(Paragraph('No de-growth products found for this distributor.',
                                PS('nod', fontSize=8, textColor=DGRAY)))
    story.append(Spacer(1, 10))

    # ── Table 2: Orders to place ──
    story.append(Paragraph(
        f'Orders to be collected and placed in {month_str} as per movement',
        PS('sh2', fontName='DVSans-Bold', fontSize=8.5, textColor=DGRAY, spaceAfter=3)))
    story.append(Paragraph(
        f'Formula: LY Monthly Avg \u2212 CY Monthly Avg + De-growth  |  De-growth = LY Monthly Avg \u2212 CY Monthly Avg  |  CY Monthly = CY YTD \u00f7 {months_elapsed} months  |  Positive \u25cf = Place Order  \u2022  Negative \u25cf = No Action',
        PS('note', fontSize=7, textColor=DGRAY, spaceAfter=5, leading=10)))

    t2_hdr = [
        ph('Product'),
        ph('LY\nAnnual Qty', TA_RIGHT), ph('LY\nAnnual Amt', TA_RIGHT),
        ph('LY\nMonthly\nAvg Amt', TA_RIGHT),
        ph('CY YTD\nQty', TA_RIGHT), ph('CY YTD\nAmt', TA_RIGHT),
        ph('CY\nMonthly\nAvg Amt', TA_RIGHT),
        ph('De-growth\n(Amt)', TA_RIGHT),
        ph('Order to\nPlace (Amt)', TA_RIGHT),
        ph('Action', TA_CENTER),
    ...
    t2_data = [t2_hdr]
    for p, q6, a6, ly_mon_a, cy_q, cy_a, cy_mon_a, deg_abs_a, order_a in orders_table:
        order_color  = '#3B6D11' if order_a <= 0 else '#A32D2D'
        action       = 'No Action' if order_a <= 0 else 'Place Order'
        action_color = '#3B6D11' if order_a <= 0 else '#A32D2D'
        deg_color    = '#A32D2D' if deg_abs_a > 0 else '#3B6D11'
        t2_data.append([
            Paragraph(p, PS(f'p2_{p[:6]}', fontSize=7, leading=9)),
            pc(str(q6), align=TA_RIGHT),
            pc(inr(a6), align=TA_RIGHT),
            pc(inr(ly_mon_a), align=TA_RIGHT),
            pc(str(cy_q) if cy_q else '\u2014', align=TA_RIGHT),
            pc(inr(cy_a) if cy_a else '\u2014', align=TA_RIGHT),
            pc(inr(cy_mon_a) if cy_mon_a else '\u2014', align=TA_RIGHT),
            pc(inr(deg_abs_a), deg_color, TA_RIGHT),
            pc(inr(order_a), order_color, TA_RIGHT),
            pc(action, action_color, TA_CENTER),
        ])

    t2 = Table(t2_data,
               colWidths=[46*mm, 12*mm, 17*mm, 16*mm, 12*mm, 16*mm, 16*mm, 16*mm, 16*mm, 16*mm],
               repeatRows=1)
...
with st.sidebar:
    st.header("Settings")
    months = st.number_input("Months elapsed in FY", min_value=1, max_value=12, value=3,
                              help="April=1, May=2, June=3...")
    today = date.today()
    ...
    logo_file = st.file_uploader("Upload Accurex logo (optional)", type=['jpg','jpeg','png'])
...
if uploaded:
    ...
    if st.button(f"Generate {len(sel_names)} PDF report(s)", disabled=len(sel_names)==0):
        ...
        for i, name in enumerate(sel_names):
            ...
            try:
                pdf_bytes = generate_pdf(name, fy26, fy27, months, today_str, month_str, logo_bytes)
